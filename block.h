...
/*
Template classes GBlock and DBlock
Implement adding and deleting items of the same type in blocks.

If there there are many items then using GBlock or DBlock
...
(3) All GBlock and DBlock functions are inline, so there are
no extra function calls.

Differences between GBlock and DBlock:
(1) DBlock allows both adding and deleting items,
whereas GBlock allows only adding items.
(2) GBlock has an additional operation of scanning
items added so far (in the order in which they were added).
(3) GBlock allows to allocate several consecutive
...
GBlock<MyType> *block = new GBlock<MyType>(BLOCK_SIZE);
...
template <class Type> class GBlock
{
public:
	...
	GBlock(int size, void(*err_function)(const char *) = NULL) { first = last = NULL; block_size = size; error_function = err_function; }

	/* Destructor. Deallocates all items added so far */
	~GBlock() { while (first) { block *next = first->next; delete[]((char*)first); first = next; } }
